...
if (isset($_GET['email_confirmation'])) {
    ...
    $query = select("SELECT * FROM `user-cred` WHERE `email`=? AND `token`=? LIMIT 1", [$data['email'], $data['token']], 'ss');
    ...
    if (mysqli_num_rows($query) == 1) {
        ...
        if ($fetch['is_verified'] == 1) {
            ...
            $update = upd("UPDATE `user-cred` SET `is_verified`=? WHERE `id`=?", [1, $fetch['id']], 'ii');
            if ($update) {
